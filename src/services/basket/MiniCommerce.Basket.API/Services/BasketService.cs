...
public sealed class BasketService : IBasketService
{
    ...
    public async Task<ServiceResult<BasketResponse>> RemoveItemAsync(
        string customerId,
        string productId,
        CancellationToken cancellationToken = default)
    {
        var validationErrors = ValidateRemoveItemRequest(customerId, productId);
        ...
        if (shoppingCart.Items.Count == 0)
        {
            ...
            return ServiceResult<BasketResponse>.Success(
                MapToResponse(new ShoppingCart
                {
                    CustomerId = customerId
                }));
        }
    ...
    private static Dictionary<string, string[]> ValidateRemoveItemRequest(
        string customerId,
        string productId)
    {
        var validationErrors = new Dictionary<string, string[]>();
